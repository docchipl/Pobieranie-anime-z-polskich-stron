...
export default async function ServiceMakiSubs(anime, episode) {
    try {
        const response = await axios.get(`https://makisubs.online/episodes/${anime}x${episode}`, {
            headers: {
        ...
        await Promise.all(Array.from(items).map(async function (x) {
            ...
            const playerInfo = await MakiSubsAPI("doo_player_ajax", x.getAttribute("data-post"), x.getAttribute("data-nume"), x.getAttribute("data-type"));
