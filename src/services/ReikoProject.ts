...
export default async function ServiceReikoProject(
  anime: string,
  ...
  try {
    const response: AxiosResponse<string> = await axios.get(
      `https://reikoproject.blogspot.com/${anime}.html`,
      {
        headers: {
    ...
    await Promise.all(
      Array.from(items).map(function (x) {
        if (!x || !x.textContent || !x.querySelector("a")) return;
        ...
        const defineNumber = Number(
          episodeNumber === null ? 9999 : episodeNumber[0]
        );
