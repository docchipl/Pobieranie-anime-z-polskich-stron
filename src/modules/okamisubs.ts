...
function OkamiSubs(anime: string, episode: string) {
  const request = axios
    .get(`https://okami-subs.pl/anime/${anime}/odcinek/${episode}`)
    .then(function (response) {
      const dom = new JSDOM(response.data, { virtualConsole });
      const episode_url_cleaning = JSON.parse(
      ...
      return {
        status: 200,
        message: 'Success',
        episode_url: episode_url_cleaning.map(function (x: any) {
          return {
            player: x.player,
            url: x.url,
          };
